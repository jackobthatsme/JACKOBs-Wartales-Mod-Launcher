...
internal static partial class LauncherCore
{
    ...
    private static Dictionary<string, ExternalWorkingFile> LoadFileBaselines(string stateDir, LauncherState state)
    {
        ...
        foreach (var m in state.ManagedFiles.Values)
        {
            if (!m.OriginalExists)
            {
                result[m.Path] = new ExternalWorkingFile(false, Array.Empty<byte>(), new List<BinaryBaselineEdit>());
                continue;
            }
            ...
            result[m.Path] = new ExternalWorkingFile(true, bytes, new List<BinaryBaselineEdit>());
        }
        return result;
    }
    ...
    private static void VerifyApplied(string gameDir, LauncherState state,
        Dictionary<string, byte[]> pakWorking,
        Dictionary<string, ExternalWorkingFile> fileWorking)
    {
        if (pakWorking.Count > 0)
        {
        ...
        foreach (var op in package.Manifest.Operations)
        {
            switch (op.Type)
            {
                ...
                case "replaceEntry":
                {
                    ...
                    if (!pakWorking.ContainsKey(entry))
                ...
                case "externalBinaryDelta":
                    ApplyExternalBinaryDeltaV030(package, state, op, fileWorking, GetStateDirectory(state.GameDirectory));
                    break;
        ...
        Dictionary<string, ExternalWorkingFile> working)
    {
        ...
        if (!string.IsNullOrWhiteSpace(op.ExpectedSha256))
        {
            ...
            if (!managed.OriginalHash.Equals(op.ExpectedSha256, StringComparison.OrdinalIgnoreCase))
                throw UnsupportedGameVersion(target);
        }

        var current = working[target];
        if (!current.Exists) throw UnsupportedGameVersion(target);
        var merged = ApplyXmlPatch(current.Bytes, package.ReadJson(op.Source));
        working[target] = new ExternalWorkingFile(true, merged, null);
    }
        ...
        Dictionary<string, ExternalWorkingFile> working)
    {
        ...
        working[target] = new ExternalWorkingFile(true, replacement, null);
    }
}
